...
int main(int argc,char**argv) {
 try {
  owlanzi::RuntimeOptions options;bool showToken=false;int duration=0;
  for(int i=1;i<argc;++i) {
   ...
   else if(arg=="--duration")duration=std::stoi(value());else if(arg=="--show-token")showToken=true;
   else if(arg=="--help") {std::cout<<"Owlanzi TC002 native app (defaults: demo, loopback port 8080)\n--demo | --live --data-dir PATH --bind IP --port PORT --ca-bundle PATH\n--show-token (display local browser pairing token) --duration SECONDS\n";return 0;}
  ...
  if(showToken)std::cout<<"Local pairing token: "<<runtime.pairingToken()<<"\n";
  else std::cout<<"Pairing token is in your data directory. Use --show-token to display it locally.\n";
